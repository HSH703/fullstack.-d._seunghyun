...
/**
 * 게시글 서비스 - 게시글 작성, 조회, 수정, 삭제 - 페이징 조회 및 해시태그 검색 - 리트윗 수 포함
 */
@Service
...
public class PostService {
	...
	public PostResponseDto createPost(Long userId, PostRequestDto dto, List<MultipartFile> files) {
		AppUser user = userRepository.findById(userId).orElseThrow(() -> new IllegalArgumentException("사용자 없음"));
		Post post = new Post();
		...
		if (dto.getHashtags() != null && !dto.getHashtags().isEmpty()) {
			Set<String> distinctTags = Arrays.stream(dto.getHashtags().split(",")).map(String::trim) // 양쪽공백빼기
					.filter(s -> !s.isEmpty()) // 비게아니라면
					.collect(Collectors.toSet());

			distinctTags.forEach(tagStr -> {
				String normalized = tagStr.startsWith("#") ? tagStr.substring(1) : tagStr;
				Hashtag tag = hashtagRepository.findByName(normalized).orElseGet(() -> {
	...
	public PostResponseDto getPost(Long postId) { // 해당하는 글번호 받아서
		Post post = postRepository.findById(postId).filter(p -> !p.isDeleted())
	...
	public PostResponseDto updatePost(Long userId, Long postId, PostRequestDto dto, List<MultipartFile> files) {
		...
		if (dto.getHashtags() != null && !dto.getHashtags().isEmpty()) {
			Set<String> distinctTags = Arrays.stream(dto.getHashtags().split(",")).map(String::trim)
