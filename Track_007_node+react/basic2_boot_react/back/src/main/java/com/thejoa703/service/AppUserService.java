...
public class AppUserService {
	...
	public UserResponseDto signup(UserRequestDto request, MultipartFile profileImage) {
		...
		user.setEmail(request.getEmail());  // 이메일 설정
		...
		user.setUfile(profileImage != null && !profileImage.isEmpty() 
				? fileStorageService.upload(profileImage)  // 업로드용 이미지 저장
    ...
    public UserResponseDto login(LoginRequest request) {
        // DB정보 : 이메일 + provider로 사용자 조회, 없으면 예외 처리
    ...
    public UserResponseDto updateNickname(Long userId, String newNickname) {
        ...
        return UserResponseDto.fromEntity(appUserRepository.save(user));  //## 3. save 변경 저장후 dto 반환
    }
    ...
    public UserResponseDto updateProfileImage(Long userId, MultipartFile profileImage) {
    	...
    	user.setUfile(profileImage != null && !profileImage.isEmpty()    // 기존의 이미지가 빈게 아니라면
    ...
    public AppUser saveSocialUser( String email, String provider, String providerId  ,String nickname, String image) {
    	AppUser user = AppUser.builder()
    						  ...
    						  .role("ROLE_USER")
    						  .build();
    	return appUserRepository.save(user);  // CRATE → insert
    }
    
	// 권한 조회
    public String findRoleByUserId( Long userId ) {
    	return appUserRepository.findById(userId)		// userId로 사용자 조회 Optional<AppUser> 반환
    						    .map(AppUser::getRole)  // 조회된 사용자가 있으면 getRole() 권한반환 / :: - 어떤 메스드(AppUser) 누구(Role.)
