...
1) 로컬회원가입 : 이메일/비밀번호 가입 
                → PasswordEncoder 암호화
                    → 로그인 JWT 발급(사용자측에 암호화해서 저장)

2) 소셜로그인 : 구글/카카오/네이버 인증 성공
              → OAuth2SuccessHandler 에서 JWT 발급

2. JWT 발급구조
1) Access Token  : 짧은 기간의 유효시간(출입증)  →  api 호출 시 사용
2) Refresh Token : 긴   기간의 유효시간(장기체류증)  →  redis 냉장고에 안전보관

3. 프론트엔드(react + next)에서 처리
1) Access Token 를 localStorage에 저장
2) Api 호출 `Authorization: Bearer <token>` 헤더에 붙임.

4. 서버처리
- `JwtAuthenticationFilter` 가 모든 요청 앞에서 토큰검증
- 검증 성공시 `SecurityContext` 사용자 정보를 저장
...
■ 2. JWT 구조 (면접문제)
1.  Header → 토큰의 머릿말 ( 이 토큰은 HS256 알고리즘으로 서명했다는 정보 )
```json
  { "alg": "HS256", "typ": "JWT" }
```

2. Payload(Claims) → 토큰의 몸통( 누가, 어떤 권한을 , 언제까지 사용가능한지~ 사용자의 신분정보 )
```json
   {
     "iss": "thejoa703",   // 발급자
     "sub": "12345",       // 사용자 ID
...
3. Signature → 토큰의 도장 ( 서버만 아는 비밀키로 찍은 도장 → 위조/변조 방지 )
```json
...
■ 3. 핵심정리
1. JWT VS 세션
- 세션 : 서버 메모리에 사용자 상태를 저장해서 가져가있는 상태 → 서버확장시 부담
                                                      (서버에서 출입명단 직접 들고 있는 것/메모리를 너무 많이 차지함.)
- JWT(Json Web Token) : 토큰 자체에 인증정보를 포함 → 확장성↑ 
                                                ( 사용자 출입증을 직접 들고다니기/ex.신분증을 들고다니는 것과 같음. )

2. Access Token VS Refresh Token 
1) Access Token  : 짧은 기간의 유효시간(출입증)  →  api 호출 시 사용
2) Refresh Token : 긴   기간의 유효시간(장기체류증)  →  redis 냉장고에 안전보관

3. Redis 사용이유?(보안회사 취준시 보안과 관련된 이슈 찾아보기~)
- 토큰냉장고 → 장기체류증 안전하게 보관, 필요시 꺼내 씀
- Refresh Token을 중앙에서 관리
- TTL(만료시간)로 자동으로 만료처리 가능
- 로그아웃 시 즉시 삭제 가능  
...
```bash
- Cmd or Powershell 사용
wsl --update → docker restart후에 아래 커멘드 입력
docker --version
docker ps
docker pull redis
docker run -d --name my-redis -p 6379:6379 redis (암기사항)
```
```bash
docker  exec  -it my-redis redis-cli (ping 입력 → pong으로 응답하면 성공)
...
1. security + jwt + redis
   - JwtProperties : 토큰
      * issue, secret, expSeconds 기본속성

   - JwtProvider   : 토큰을 발급/검증
      * Access Token (출입증), Refresh Token 생성, 파싱

   - TokenStore    : 토큰 냉장고(저장)
      * Redis  저장소 , Refresh Token refresh:<userId>

   - JwtAuthenticationFilter : 보안 게이트
      * 매 요청마다 `Authorization: Bearer <token> 헤더 확인 → 토큰검증 → SecurityContext에 사용자 정보저장
