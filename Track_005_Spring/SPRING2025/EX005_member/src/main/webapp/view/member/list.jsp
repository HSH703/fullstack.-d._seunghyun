...
<div class="show-result container card  my-5 p-4">
	<form>
		...
		<div class="mb-3">
			<label class="form-check-label" for="mbtiTypeId">MBTI TYPE :
			</label> <select name="mbtiTypeId" id="mbtiTypeId" class="form-control">
		...
		<input type="button" value="USER 데이터 수정" class="btn btn-primary"
			title="USER 데이터수정" id="updateUser" />
	</form>
</div>
<script>
	$(function(){
	...
	function userList(){
		$.ajax({
			url:"selectAll",  //무조건 index.jsp에서 실행.
			type:"GET",
	...
	function userListResult(json){
		...
		$.each(json, function (idx, user){  //json(전체), function (idx 순번 , user 한개씩) 
			$("<tr>")  //tr태그만들기
				.append( $("<td>").html(total-idx) ) //데이터붙이기  (append-끝에 ) / (html-reset 지우고 넣기)
	...
	function userSelect(){
		// 수정버튼클릭시 - 해당유저의 정보를 form 태그안에 넣기	
		//$(".updateUser").on();
	...
	function userDelete(){
		...
		$("body").on("click", ".deleteUser", function(){  //userList는 ajax로 처리하고나서 찾기
			//	내가선택한 deleteUser  가장가까운 tr    .hidden_id	     	값
			let appUserId = $(this).closest("tr").find(".hidden_id").val();
