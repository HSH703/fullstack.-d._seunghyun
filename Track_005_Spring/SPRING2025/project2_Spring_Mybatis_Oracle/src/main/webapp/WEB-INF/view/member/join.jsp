...
<div class="container mt-5">
	...
	<script>
		$(function() {
			//1. 대상찾아오기 id가 email의 값을 찾아오려고함.
			console.log("1. " + (0 == false)); //true 자동변환
			console.log("2. " + (0 == false)); //false 자료형까지
			console.log("3. " + (0 == "")); //true 자동변환
			console.log("4. " + (0 === "")); //false 자동변환

			$("#email")
					.on(
							"keyup",
							function() { //click, keyup ,,,
								...
								if (keyword === "") { // 만약 빈칸인지 검사
									$(".iddouble_result")
											.empty()
											.append(
													"<span class='text-danger p-3'>이메일을 입력해주세요.</span>");
									return;
								} else { // 아니라면 ajax연동 - iddouble / POST / enail /
									$
											.ajax({
												...
												success : function(res) { //res = {cnt: 1}
													...
													if (res.cnt == 1) {
														$(".iddouble_result")
																.empty()
																.append(
																		"<span class='text-danger p-3'>이미 사용중인 이메일입니다.</span>");
													} else {
														$(".iddouble_result")
																.empty()
																.append(
																		"<span class='text-success p-3'>사용가능한 이메일입니다.</span>");
													}
												},
												error : function() {
													$(".iddouble_result")
															.empty()
															.append(
																	"<span class='text-danger p-3'>서버요청중 오류가 발생했습니다.</span>");
												}
											});
								}
							});
