...
public class Mini_Music_Player {
	//멤버변수 
	private List<UserInfo> users; //add, get, size, remove, contains
	private MMPController[] controller;
	
	public Mini_Music_Player() {
		users	   = new ArrayList<>();
		controller = new MMPController[]{
				new Login(), new Add(), new Show(),
				new Input(), new Output(),  new Delete()
		};
	}
	//멤버함수(메뉴판)
	public void menu() {
		...
		*/
		...
		while(num != 9) { //9가 아니라면 계속.
			...
			System.out.println("🎵🎧WELCOME TO Mini_Music_Player🎧🎵"+"\n[1] ➕ 계정 생성" + "\n[2] 🔍 계정 조회" +
						       "\n[3] 💵 음악 입력" + "\n[4] 💸 음악 출력" + "\n[5] 🗑️ 계정 삭제 " 
						       	+ "\n👉 번호를 선택하세요:");
			num=scanner.nextInt();
...
//			 if (num == 9) {System.out.println("종료합니다.");}
			if(num>=2 && num<=5) {
				find=controller[0].exec(users,0);
				if(find == 0) {System.out.println("유저정보를 확인해주세요~!"); continue;}
				}
...
/////////////////////////////////////////////////////////////	
	public static void main(String[]args) {
		new Mini_Music_Player().menu();		//start
			//출력
