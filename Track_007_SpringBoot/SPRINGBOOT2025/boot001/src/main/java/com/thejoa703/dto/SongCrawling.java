	...
	public List<SongDto> crawling(){
		...
		try {
			...
			Connection conn = Jsoup.connect(url)
								   ...
								   .referrer("https://www.melon.com/") //출처 ## (com 까지만 입력)
								   .timeout(10_000);  // 연결 시도후에 응답을 기다리는데 최대시간 (밀리초) 10초 (15초~ 이상이 되면 브라우저 연결안됨.)
			Document doc = conn.get();
			
			Elements titleEle  = doc.select(".ellipsis.rank01 a");
			...
			for(int i=0; i<cnt; i++) {
				...
				//String titleeHres = titleEle.get(i).attr("href");
...
- 스크랩핑 : 수집된 페이지에서 원하는 데이터만 추출하는 과정
