...
public class AppUserSecurityController {
	...
	/////마이페이지
	@RequestMapping("/mypage") //마이페이지 - 로그인정보 Principal (일회용)
	public String mypage( Principal principal , Model model ) { 
		model.addAttribute("dto", service.selectEmail(principal.getName())); //## / 권한 없으면? 로그인 X 
																			 // principal은 로그인할때 사용
	...
	public String update( @RequestParam("file") MultipartFile file, AppUser dto, RedirectAttributes rttr) { 
		...
		if( service.update(file, dto) > 0 ) { result="수정성공"; }
		rttr.addFlashAttribute("success", result);
	...
	public String delete(AppUser dto, RedirectAttributes rttr, 
		...
		String result = "비밀번호를 확인해주세요.";
		if( service.delete(dto) > 0 ) { // 삭제 성공 → 로그아웃처리
