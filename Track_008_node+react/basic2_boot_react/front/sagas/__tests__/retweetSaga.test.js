...
describe('retweetSaga', () => {
  ...
  it('addRetweetFailure', async () => {
    api.post.mockRejectedValue(new Error('fail'));   // 서버에서 받아오는 값
    const dispatched = [];
  ...
  // --- 리트윗 여부 확인 ---  hasRetweetedRequest, hasRetweetedSuccess, hasRetweetedFailure,
  ...
  // --- 내가 리트윗한 글 목록  ---  fetchMyRetweetsRequest, fetchMyRetweetsSuccess, fetchMyRetweetsFailure,
  it('fetchMyRetweetsSuccess', async () => {
    ...
    expect(dispatched).toContainEqual(fetchMyRetweetsSuccess( {1:true , 2 : true} ));
  });
