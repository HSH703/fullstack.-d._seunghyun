...
describe( 'user saga', ()=>{
        ...
        ///////////////////////
        it( 'signup success', ()=>{
            const action = {type: SIGN_UP_REQUEST};   // #1. 액션타입
            const gen = signUp(action);               // #2. logout 기능확인

            expect(gen.next().value).toEqual(call(signUpApi , action.data ));   // #3. 결과물확인
            expect(gen.next({}).value).toEqual(put({type: SIGN_UP_SUCCESS }));   // #3. 결과물확인
        ...
        it( 'loadUsers success', ()=>{
            ...
            const gen = loadUsers(action);               // #2. logout 기능확인
            ...
            expect(gen.next({data: fakeUsers}).value)
                .toEqual(put({type: LOAD_USERS_SUCCESS , data:[ {id:1, email:'z@z', nickname:'zzz'} ] }));   // #3. 결과물확인
        ...
        it( 'updateNickname success', ()=>{
            ...
            const gen = updateNickname(action);               // #2. logout 기능확인

            expect(gen.next().value).toEqual(call(updateNicknameApi, action.data));   // #3. 결과물확인
            expect(gen.next({}).value).toEqual(put({type: UPDATE_NICKNAME_SUCCESS , data: {id:1, nickname:'new'} }));   // #3. 결과물확인
        ...
        it( 'deleteUser success', ()=>{
            ...
            const gen = deleteUser(action);               // #2. logout 기능확인

            expect(gen.next().value).toEqual(call(deleteUserApi, action.data.id));   // #3. 결과물확인
            expect(gen.next({}).value).toEqual(put({type: DELETE_USER_SUCCESS , data: {id:1} }));   // #3. 결과물확인
