...
@SpringBootTest
@Transactional    //테스트 실행중 발생하는 db변경 자동 롤백
class Boot001ApplicationTests2_User { 
	...
	public void testCrudAppUserAuth() {
		...
		user.setEmail("2@2"); user.setPassword("1"); user.setMbtiTypeId(1);
		...
		update.setMbtiTypeId(2);
		update.setNickname("nickname002"); update.setAppUserId(1);
		//System.out.println( "...... 6." + update);  
		System.out.println( "...... 6." + dao.updateAppUser(update));  

		//7. 사용자삭제
		AppUserDto delete = new AppUserDto(); delete.setAppUserId(1);
		System.out.println( "...... 7." + dao.deleteAppUser(delete));

		//8. 권한삭제
		AuthDto dauth = new AuthDto();  dauth.setEmail("1@1");
		System.out.println( "...... 8." + dao.deleteAuth(dauth));  //1
	}
}
