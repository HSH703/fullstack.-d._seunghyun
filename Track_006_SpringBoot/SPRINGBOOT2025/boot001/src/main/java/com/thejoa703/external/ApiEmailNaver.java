...
public class ApiEmailNaver {
	...
	public void sendMail(String subject, String content, String email) {
		...
		String user     = "hsh201008@naver.com";  //네이버아이디@naver.com
		String password = "4BD5EE77VU8Y";  //2단계인증 비밀번호
	    ...
	    props.put("mail.smtp.starttls.enable", "true");  	// 이메일전송서 보안연결
	    props.put("mail.smtp.ssl.trust", "smtp.naver.com"); // ssl인증서연결
	    ...
	    try {
	    	message.setFrom(new InternetAddress(user));  //보내는 사람 hsh201008@naver.com
	    	message.setRecipient(Message.RecipientType.TO, new InternetAddress(to));  //받는사람
	    	...
	    	message.setContent("" 
	    		    ...
	    		    + "</div>"
	    		    , "text/html; charset=euc-kr");  //내용 바꿔서 넣어도 됨.
